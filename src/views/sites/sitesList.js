...
export default function useSitesList() {
  ...
  const refSitesListTable = ref(null)

  // Table Handlers
  const tableColumns = [
    { key: 'id', label: '#', sortable: true },
    { key: 'sitesStatus', sortable: true },
  ...
  const dataMeta = computed(() => {
    const localItemsCount = refSitesListTable.value ? refSitesListTable.value.localItems.length : 0
    return {
  ...
  const refetchData = () => {
    refSitesListTable.value.refresh()
  }
  ...
  const resolveSitesStatusVariantAndIcon = status => {
